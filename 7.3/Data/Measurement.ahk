...
;###Molo-Image##############################################################
		...
		CounterImageStray :=1												;Set Counter for single stray light Image to beginning
		CounterImageFluore :=1												;Set Counter for single fluorecence light Image to beginning
		While CounterImageMolo <= nbrOfImages								;Record number of Sub-Images per Array
			...
			If StrayImage = 1												;Checks if Starylight Image is desired
				...
				CounterImageStray+=1
			;###Making single surface image in fluorecence light mode#############
			If FluorImage = 1												;Checks if Fluorecence Image is desired
				{
					if CounterImageFluore = %nbrOfImagesFluor%									
					{
					#IncludeAgain Data/Image-Fluor.ahk
					CounterImageFluore :=0
					}
				}
				CounterImageFluore+=1
			;###############################################################
					;###Pausing the Script############################
